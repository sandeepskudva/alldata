...
public class testcase012 extends driverScript  {

	public void case012(ExtentReports report,ExtentTest logger,int Rownum) throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException, SQLException 
	{
		...
		if(status)status=ep.securitychangeQuestions(report, logger, Rownum);	
		
		if(status)status=ep.securitychangePassword(report, logger, Rownum);
	}
