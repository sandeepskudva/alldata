...
public class testcase017 extends driverScript  {

	public void case017(ExtentReports report,ExtentTest logger,int Rownum) throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException, SQLException 
	{
		...
		if(status)status=atv.reportStolen(report, logger);
		if(status)status=atv.changeStatus(report, logger,"Stolen","Y");
	}
