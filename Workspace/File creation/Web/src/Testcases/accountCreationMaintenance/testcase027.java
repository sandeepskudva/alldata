...
public class testcase027 extends driverScript  {

	public void case027(ExtentReports report,ExtentTest logger,int Rownum) throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException, SQLException 
	{
		...
		if(status)status=vp.violationSelection(report, logger, Rownum);
		if(status)status=vp.payViolationCash(report, logger, Rownum);
